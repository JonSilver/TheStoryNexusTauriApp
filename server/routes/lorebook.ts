...
export default createCrudRouter({
  ...
  customRoutes: (router, { asyncHandler, table }) => {
    ...
    router.get('/global', asyncHandler(async (_, res) => {
      const entries = await db.select()
        .from(table)
        .where(eq(table.level, 'global'))
    ...
    router.get('/series/:seriesId', asyncHandler(async (req, res) => {
      const entries = await db.select()
        ...
        .where(
          and(
            eq(table.level, 'series'),
            eq(table.scopeId, req.params.seriesId)
          )
        )
        .orderBy(table.createdAt);
      res.json(entries.map(transform));
    ...
    router.get('/story/:storyId/hierarchical', asyncHandler(async (req, res) => {
      ...
      if (story.seriesId) {
        conditions.push(
          and(
            eq(table.level, 'series'),
            eq(table.scopeId, story.seriesId)
          )
        );
      }
    ...
    router.get('/global/export', asyncHandler(async (_, res) => {
      const entries = await db.select()
        .from(table)
        .where(eq(table.level, 'global'))
        .orderBy(table.createdAt);
    ...
    router.post('/global/import', upload.single('file'), asyncHandler(async (req, res) => {
      if (!req.file) {
